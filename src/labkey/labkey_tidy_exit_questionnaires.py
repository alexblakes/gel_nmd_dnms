...
_FILE_IN = "data/raw/gmc_exit_questionnaire_2023-08-01_09-42-17.tsv"
_FILE_OUT_SOLVED = "data/interim/labkey_exit_questionnaires_case_solved.tsv"
_FILE_OUT_ACMG = "data/interim/labkey_exit_questionnaire_acmg.tsv"
...
def main():
    ...
    eq = read_exit_questionnaire(_FILE_IN)

    case_solved = get_case_solved(eq)
    acmg = get_exit_questionnaire_acmg(eq)

    logger.info("Writing to output.")
    case_solved.to_csv(_FILE_OUT_SOLVED, sep="\t", index=False)
    acmg.to_csv(_FILE_OUT_ACMG, sep="\t", index=False)
